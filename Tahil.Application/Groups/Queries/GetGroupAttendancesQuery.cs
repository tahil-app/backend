...
public record GetGroupAttendancesQuery(int Id, int Year) : IQuery<Result<List<GroupDailyAttendance>>>;
...
public class GetGroupAttendancesQueryHandler(IGroupRepository groupRepository, IApplicationContext applicationContext) : IQueryHandler<GetGroupAttendancesQuery, Result<List<GroupDailyAttendance>>>
{
    ...
    {
        return await groupRepository.GetGroupAttendancesAsync(request.Id, request.Year, applicationContext.TenantId);
    }
}